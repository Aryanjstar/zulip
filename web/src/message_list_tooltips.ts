...
export function initialize(): void {
    ...
    message_list_tooltip(".rendered_markdown a.message-link", {
        ...
        onShow(instance) {
            ...
            const full_url = href.startsWith("#")
                ? window.location.origin + "/" + href
                : href;
            const channel_topic = hash_util.decode_stream_topic_from_url(full_url);
            if (
                channel_topic === null ||
                channel_topic.message_id === undefined
            ) {
                return false;
            }
            ...
            const content_preview =
                text_content.length > 200
                    ? text_content.slice(0, 200) + "…"
                    : text_content;
            instance.setContent(
                parse_html(
